...
if __name__ == "__main__":
    ...
    for resolution in [50, 100, 200, 400]:
        result = calc3.grid_method(grid_resolution=resolution, samples_per_cell=16)
        true_area = calc3.analytical_area()
        error = abs(result['area'] - true_area) / true_area * 100
        print(f"Resolution {resolution:3d}×{resolution:3d}: Area = {result['area']:8.4f}, "
              f"Error = {error:5.2f}%, Time = {result['computation_time']:.3f}s")
